...
const BasketMobile = () => {
 ...
 const goToCheckout = () => {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper', display: 'flex', flexDirection: 'column' }}>
      {basketItems && basketItems.map((x, id) => (
        <P.BasketWrapperMobile> 
          <Box sx={{ my: 3, mx: 2 }}>
            <Grid container alignItems="center">
              <Grid item xs>
                <Typography sx={{ fontSize: 17 }} gutterBottom variant="h4" component="div">
                  {x.title}
                </Typography>
              </Grid>
              <Grid item>
            ...
            <Typography sx={{ display: 'flex', justifyContent: 'center' }} color="text.secondary" variant="body2">
              <P.ProductPhoto
                src={x.image}
                loading="lazy"
                alt=""
      ...
      <TableRow
        ...
        <TableCell>
        <P.ButtonWrapper>
          <Button onClick={goToCheckout} variant="contained" startIcon={<ShoppingBasketIcon />}>
            Proceed
            </Button>
        </P.ButtonWrapper>
        </TableCell>
